...
import bs4
import requests
from graphql import ProfileData, ProfileInfoData, DiveStatistic
...
class ProfileParser:
    ...
    def __wrapLooseText(self, text: str) -> str:
        try:
            ...
            print(f"Failed to parse text input due to the following error: {repr(exc)}")
    ...
    def __assignInfoKeys(self, dct: dict[str, str]) -> Optional[ProfileInfoData]:
        ...
        for key, value in dct.items():
            if "Name:" in key:
                ...
                if foundErrors:
                    print(f"ValueErrors found for DiveMeets ID {value}")
    ...
    def __parseInfo(self, data: bs4.element.Tag) -> Optional[ProfileInfoData]:
        ...
        body = soup.find("body")
        if body is None:
            print("parseInfo: body not found")
            return None
    ...
    def __parseDiveStatistics(
        ...
        try:
            ...
            for row in rows:
                ...
                if len(subRow) < 6:
                    print("parseDiveStatistics: length of subrows is less than 6")
                    return None
                ...
                try:
                    ...
                    print(f"ValueError in parseDiveStatistics: {repr(exc)}")
                    return None
                except Exception as exc:
                    print(f"Found exception in parseDiveStatistics: {repr(exc)}")
                    return None
    ...
    def parseProfile(self, link: str) -> bool:
        try:
            ...
            if len(tables) == 0:
                print("parseProfile: tables list is empty")
                return False
            ...
            for elem in list(filter(lambda x: "img" not in x, htmlComponents)):
                ...
                if body is None:
                    print("parseProfile: Info page body not found")
                    return False
            ...
            print(f"Found exception in parseProfile: {repr(exc)}")
            return False
