...
def get_reports_objects(date):
    response = s3client.list_objects_v2(
        Bucket=os.environ["bucket"], Prefix=f"public/reported-posts/{date}"
    )
    ...
    return {"date": date, "result": list(map(lambda x: x["Key"], result))}
...
def lambda_handler(event, context):
    ...
    # Provide table when there are values to display, otherwise empty message
    if reports["result"] != []:
        ...
        body = "No reported posts found for the specified date"
